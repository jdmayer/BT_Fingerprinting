...
There are different ways to apply Cross-Browser fignerprinting. In 2011 Boda et al. used a part of the ip to create a fingerprint. In 2017 Cao et al. proposed a better option.

\subsubsection{How it works}
Cross-Browser fingerprinting uses browser-independent features to be able to track a user regardless of what browser they are using. This can be done by relying only on the use of JavaScript and font detection. The technique used differs. It e.g. can rely on browser dependent plug-ins like Adobe Flash or Java to obtain a font list.(\textcite{upi15}, pp.2)

--create a unique identifier from specific browser data using JavaScript and server-side algorithms
(\textcite{boda11}, p.4)

In 2011 a study conducted by Boda et al. discovered that most user can be tracked across the 5 most populat web browsers using

 a part of the ip address, time zone, screen resolution and the availability of a specific font set.(\textcite{boda11}, p.1)
...
ip version - unavailable behind an anonymous network or a proxy (\textcite{Cao17}, p.1)

\newpage
A technique developed by Yinzhi Cao
**	E.g. used screen’s ratio of width to height as it remains consistent even when using zoom (\textcite{Cao17}, p.2 )
...
**	99,24\%  successfully identify (\textcite{Cao17}, p.2)

This fingerprinting technique is based on different novel operating system and hardware level features (e.g. from graphic card, CPU, audio stack, and installed writing scripts). All in al 29 features. Many of them are exposed to JavaScript over web browser APIs, which are used to extracted them.(\textcite{Cao17}, p.1)
