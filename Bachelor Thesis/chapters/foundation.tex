...
\begin{itemize}
	...
	\item Decrease the verbosity of plugin versions and the user agent string (\textcite{boda11}, p.16);\\
...
-- browser plugin
-- provides different ways of delivering rich media content
-- which traditionally not displayed using HTML
 (\textcite{nikiforakis13}, p.3)
...
-- used for retrieving more sensitive information
-- incl. ability to detect HTTP proxies 
(\textcite{nikiforakis13}, p.2)

-- proprietary browser plug-in
-- reimplements certain APIs existing in browser + accessible through js
-- APIs not always provide same results (more detailed)
-- e.g. querying a browser about the platform of
execution, Firefox reports “Linux x86 64”. Flash, on the
other hand, provides the full kernel version, e.g., Linux
3.2.0-26-generic.
(\textcite{nikiforakis13}, p.2)

-- needed to play Flash animations
-- was most commonly used video format on the www
-- therefore - expansion - compulsory
-- HTML5 - now a serious and safer alternative - adobe still installed on various browsers
(\textcite{web17})

\subsection{HTML 5 and Canvas}\label{html5}
...
\autoref{DecisionTree} gives a good overview how such a tree can be structured. The leaf nodes pose as browsers, the inner nodes display the used tests and the edges indicate the success. (\textcite{mulazzani13}, p.4)
\begin{figure}[H]
	\centering
